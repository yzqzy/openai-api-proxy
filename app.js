...
app.all(`*`, async (req, res) => {
  ...
  async function process_buffer(res) {
    ...
    if (sentence) {
      if (sentence === '[DONE]') {
        ...
        if (sentence_content) {
          const params = {
            Content: Buffer.from(sentence_content).toString('base64')
          };
          ...
          if (md_check) {
            ...
            forbidden_array.choices[0].delta.content =
              '这个话题不适合讨论，换个话题吧。';
  ...
  try {
    ...
    if (
      ...
      if (moderation && mdClient) {
        const params = {
          Content: Buffer.from(data.choices[0].message.content).toString(
            'base64'
          )
        };
        const md_result = await mdClient.TextModeration(params);
